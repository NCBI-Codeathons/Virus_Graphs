...
# Creating graph file and add metadata
...
with open(args.out+'.tsv') as f:
   for row in f:
       ...
       # Overwrite node(s) and add metadata
       G.add_node(row[1],host='human', taxid=1234)

nx.write_gexf(G, args.out+'.gexf')
nx.drawing.nx_pydot.write_dot(G, args.out+'.dot')
